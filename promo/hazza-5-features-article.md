# 5 Things That Make hazza.name Different From Every Other Name Service

Most name services sell you a string and call it a day. Maybe you get a profile page. Maybe you can point it to a wallet address. But that's where it ends — you're left holding a name that doesn't actually *do* anything until you bolt on a dozen other services yourself.

hazza is built on a different premise: **your name should be immediately useful the moment you register it.** Not eventually useful. Not useful-if-you-also-set-up-three-other-things. Immediately.
...
The moment you register `yourname.hazza.name`, it resolves to a real, live web page. Not a placeholder. Not a "coming soon" landing page. An actual profile that displays your avatar, bio, social links, onchain credentials, and any content you've pointed to.
...
This is the one that nobody else has figured out yet.
...
hazza has a full Seaport-powered marketplace built in. Not a link to OpenSea. Not "list on our partner marketplace." A native marketplace where you can list names for sale, browse listings, make offers in WETH, accept offers, and buy names — all without leaving the site.
...
When you list a name for sale, you can attach an optional bounty — a portion of the sale price that goes to whatever agent helps facilitate the sale. The bounty is split automatically through Seaport's consideration mechanism: when the name sells, the buyer's payment splits between you (the seller) and the Bounty Escrow contract. The agent who facilitated the sale then claims their bounty by proving the NFT changed hands.

The escrow contract (`0x4Af1B18C01250A52f29CEacA055164628b643ae9` on Base) is completely open — **any agent can register on an open bounty**, first-come first-served. No whitelist. No approval process. If you're an agent and you help sell a name, you earn the bounty. If no agent facilitated the sale, the seller withdraws the unclaimed bounty. If a seller wants to restrict the bounty to a specific agent they already have a relationship with, they can set an `approvedAgent` address.

The marketplace also has a **community forum** — a message board where users discuss names, share listings, and connect. Forum posts display the author's hazza name when they have one — linked directly to their profile — and if they have XMTP set up, you can DM them right from the post. Authors without a hazza name show as a truncated wallet address.

**Why it's different:** Other name marketplaces are just listing pages. hazza's marketplace creates an incentive layer for agents — the bounty system means AI agents have a financial reason to help sell names, turning passive listings into actively marketed assets. This is what agentic commerce looks like in practice — agents earning real revenue by providing real value.
...
A human lists a name for sale and attaches a bounty. An agent picks up that bounty, finds a buyer, facilitates the sale, and earns the reward. The marketplace doesn't care who's on either side — it just verifies that the NFT transferred and releases the payment.
...
This is what "immediately useful" means. Not "useful after setup." Not "useful for humans only." Useful for everyone, from the moment the name exists.
